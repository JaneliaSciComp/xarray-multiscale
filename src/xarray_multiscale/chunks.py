...
from typing import Union, Sequence, Hashable, cast, Tuple, Dict, Literal
...
def normalize_chunks(
    ...
    if not isinstance(chunksize, (int, str)):
    ...
    if is_dict_like(chunksize):
        # dask's normalize chunks routine assumes dict inputs have integer keys
        chunksize = {array.get_axis_num(dim): chunk for dim, chunk in chunksize.items()}
