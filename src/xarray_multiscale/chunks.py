...
def normalize_chunks(
    ...
    if not isinstance(chunksize, (int, str, dict)):
        if len(chunksize) != array.ndim:
            raise ValueError(
                f"Incorrect number of chunks. Got {len(chunksize)}, expected {array.ndim}"
            )
    ...
    if is_dict_like(chunksize):
        ...
        if len(chunksize.keys() - set(array.dims)) > 0:
            ...
            raise ValueError(
                f"Keys of chunksize must be a subset of array dims. Got extraneous keys: {extra}"
            )
        _chunksize = dict(zip(range(array.ndim), map(tz.first, array.chunks)))
        _chunksize.update(
            {array.get_axis_num(dim): chunk for dim, chunk in chunksize.items()}
        )
        chunksize = _chunksize

    old_chunks = map(tz.first, array.chunks)
    new_chunks = map(
        tz.first, da.core.normalize_chunks(chunksize, array.shape, dtype=array.dtype)
    )
